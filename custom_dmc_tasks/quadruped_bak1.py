...
def make_model(floor_size=None, terrain=False, rangefinders=False,
  ...
  root_dir = os.path.dirname(os.path.dirname(__file__))
  if color:
    xml_string = common.read_model(os.path.join(root_dir, 'custom_dmc_tasks1', 'quadruped_color.xml'))
  else:
    xml_string = common.read_model(os.path.join(root_dir, 'custom_dmc_tasks1', 'quadruped.xml'))
