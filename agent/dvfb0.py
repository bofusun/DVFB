...
class dvfb0Agent:
    ...
    def infer_meta(self, replay_iter, step):
        ...
        while batch_size < self.num_inference_steps:
            ...
            next_goal = next_obs
            obs_list.append(obs)
            next_obs_list.append(next_goal if self.goal_space is not None else next_obs)
    ...
    def infer_meta_from_obs_and_rewards(self, obs, next_obs, reward):
        ...
        # reward = reward[idx]
        # filter out small reward
        idx = torch.where(reward >= torch.quantile(reward, 0.98))[0]
        obs = obs[idx]
    ...
    def update_actor(self, obs, z, step):
        ...
        if self.reward_free:
            ...
            # ########
            # # 110-112         无奖励映射           5*Q+MQ
            # Q = 5*Q + MQ 
            
            # ########
            # # 120-122         无奖励映射           10*Q+MQ
            # Q = 10*Q + MQ 
            
            #######
            # 130-132         无奖励映射           50*Q+MQ
            Q = 50*Q + MQ 
