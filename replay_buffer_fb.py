...
class ReplayBuffer(IterableDataset):
    ...
    def _sample(self):
        ...
        idx = np.random.randint(0, episode_len(episode) - 2*self._nstep + 1) + 1
        ...
        for spec in self._storage._meta_specs:
            ...
            next_meta.append(episode[spec.name][idx])
        obs = episode['observation'][idx - 1]
        future_obs = episode['observation'][idx + 2*self._nstep - 1]
        action = episode['action'][idx]
        next_obs = episode['observation'][idx + self._nstep - 1]
        reward = np.zeros_like(episode['reward'][idx])
        discount = np.ones_like(episode['discount'][idx])
        if idx == episode_len(episode):
            done = np.ones_like(episode['discount'][idx])
        else:
            done = np.zeros_like(episode['discount'][idx])
        for i in range(self._nstep):
            step_reward = episode['reward'][idx + i]
            reward += self._discount ** i * step_reward
            # reward += discount * step_reward
            discount *= episode['discount'][idx + i] * self._discount
        return (obs, action, reward, discount, next_obs, future_obs, *meta)
