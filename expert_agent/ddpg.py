...
class Encoder(nn.Module):
    def __init__(self, obs_shape):
        ...
        self.repr_dim = 32 * final_shape * final_shape
...
class Critic_C(nn.Module):
    def __init__(self, obs_type, obs_dim, action_dim, feature_dim, hidden_dim, num_bins, min_value, max_value, sigma, v_expand, device):
        super().__init__()
        ...
        self.sigma = sigma * (self.max_value - self.min_value) / num_bins
        self.min_value -= v_expand / 2
        self.max_value += v_expand / 2
        self.support = torch.linspace(min_value, max_value, num_bins + 1, dtype=torch.float32).to(device)
        if obs_type == 'pixels':
            # for pixels actions will be added after trunk
            self.trunk = nn.Sequential(nn.Linear(obs_dim, feature_dim),
                                       nn.LayerNorm(feature_dim), nn.Tanh())
        ...
        def make_q():
            ...
            q_layers += [nn.Linear(hidden_dim, num_bins)]
            return nn.Sequential(*q_layers)
    ...
    def forward(self, obs, action):
        ...
        q1_prob = F.softmax(q1_logits, dim=1) 
        q2_prob = F.softmax(q2_logits, dim=1) 

        q1 = self.transform_from_probs(q1_prob)
        q2 = self.transform_from_probs(q2_prob)
        
        return q1_logits, q2_logits, q1, q2

    def loss(self, logits, target):
        # print("logits", logits.shape)
        # print("self.transform_to_probs(target)", self.transform_to_probs(target).shape)
